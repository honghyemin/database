...
-- 인덱스 생성 : BOOK 테이블의 출판사 (PUBLISHER) 컬럼에 IX BOOK 인덱스 만들기
...
-->> PRICE는 전체로 놓고 봤을 때는 정렬이 된 것이 아니다. 동일한 출판사 내(첫번째 데이터)에서만 정렬됨.

SELECT * FROM BOOK WHERE PUBLISHER = '대한미디어';
SELECT * FROM BOOK WHERE PRICE >10000;

-- WHERE 절에서 PUBLISHER, PRICE 동시 사용 시 IX_BOOK2 인덱스가 제대로 적용됨
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE = 7000;
-->> 굿스포츠 내에서 가격에 맞는 것을 찾으므로 결과를 찾게되면 종료됨. 속도면에서 좋음.
SELECT * FROM BOOK WHERE PRICE=7000 AND PUBLISHER = '굿스포츠';

-- 인덱스가 있는데 사용이 안되는 경우
SELECT * FROM BOOK WHERE PUBLISHER LIKE '대한%'; -- 인덱스 사용 (구간검색)
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -- 인덱스 적용X (FULL SCAN, 전체비교)
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; -- 인덱스 적용X (FULL SCAN, 전체비교)


-- 원본 데이터를 가공(변경)하는 경우 ( 인덱스 적용 안됨 )
