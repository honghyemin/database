...
-- UNION ALL으로 합하기(중복데이터 모두 표시(사용))
SELECT CUSTID, NAME FROM CUSTOMER
WHERE CUSTID IN (1, 2, 3)
UNION ALL
SELECT CUSTID, NAME FROM CUSTOMER 
WHERE CUSTID IN (3, 4, 5)
ORDER BY NAME; -- ORDER BY 절은 맨 마지막에 사용.

------------------------------------
-- MINUS : 차집합 ( 빼기 연산) -> 중복데이터 제거
SELECT CUSTID, NAME FROM CUSTOMER
WHERE CUSTID IN (1, 2, 3)
MINUS
SELECT CUSTID, NAME FROM CUSTOMER 
